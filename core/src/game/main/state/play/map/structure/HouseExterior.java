    ...
    public void generate(Random rand, World map, int x, int y, int z) {
        for (int xx = -1; xx < w + 1; xx++) {
            for (int yy = -1; yy < h + 1; yy++) {
                ...
                if (t != null && t.name.equals(map.biomes[(x + xx) / 16][(y + yy) / 8].wall.tile())) {
                    ...
                    map.tiles.placeTile(Game.TILES.load(map.biomes[x / 16][y / 8].ground), x + xx, y + yy, 0);
                }
            }
        }
        ...
        for (int xx = 0; xx < w; xx++) {
            for (int yy = 0; yy < h; yy++) {
                map.tiles.erase(x + xx, y + yy);
                map.tiles.placeTile(Game.TILES.load(wall), x + xx, y + yy, 0);
                map.tiles.placeTile(Game.TILES.load(roof), x + xx, y + yy, 1);
            }
        }
        ...
        map.tiles.placeTile(Game.TILES.load(entrance), x + ex, y + ey, 0);

        map.tiles.erase(x + ex, y + ey - 1);
        map.tiles.placeTile(Game.TILES.load(map.biomes[(x + ex) / 16][(y + ey - 1) / 8].ground), x + ex, y + ey - 1, 0);
